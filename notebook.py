...
#importing panda, numpy and matplotlib libraries
import pandas as pd
import numpy as np
...
#reading the office data set 
office_data = pd.read_csv('datasets/office_episodes.csv')

#using head to view the first 20 items in the dateset
office_data.head(20)
...
#using info() to get more information about each columns and their datatype
office_data.info()
...
#using df.iterrows(), we will group the ratings into specfic colours based on the scaled_ratings row ratings

#If Ratings < 0.25 are colored "red"
#If Ratings >= 0.25 and < 0.50 are colored "orange"
#If Ratings >= 0.50 and < 0.75 are colored "lightgreen"
#If Ratings >= 0.75 are colored "darkgreen"


column = []
    
for i, r in office_data.iterrows():
    if r['scaled_ratings'] < 0.25:
        column.append('red')
    elif r['scaled_ratings'] < 0.5:
        column.append('orange')
    elif r['scaled_ratings'] < 0.75:
        column.append('lightgreen')
    else:
        column.append('darkgreen')                 
...
#we use df.iterrows() to group the size marker into two sizes based on whether there was a guest appearance on the episode

marker = []

for i, r in office_data.iterrows():
    if r['has_guests'] == False:
        marker.append(25)
    else: 
        marker.append(250)
...
plt.scatter(x=office_data['episode_number'], y=office_data['viewership_mil'], c=column, s=marker)
...
#counting values in guest stars to determine how many actors have guest stared in office episodes, the number turns out to be 29
office_data['guest_stars'].count()


# In[23]:


#using value_counts gives us a complete list of the actors who have guest starred on the office episodes
office_data['guest_stars'].value_counts()


# In[24]:


#filtering for every office episode that had over 20 million viewers
office_data[office_data['viewership_mil'] > 20]


# #### We can conclude that the highest viewed epsiode in The Office was in season 5 and is episode 77 titled "Stress Relief". This episode features Cloris Leachman, Jack Black and Jessica Alba as guest stars
